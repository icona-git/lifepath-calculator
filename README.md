...
costs (Calgary, Alberta, 2026-style assumptions), see the gross income that supports it, then compare
Safe / Balanced / Bold pathway plans — with AI exposure, disruption risk, and AI leverage shown for
every recommended career path.
...
| `data/cost_assumptions.json` | Every monthly cost tier (housing, vehicle, food, …) + misc buffer % |
...
1. **Budget** — tier choices map to monthly line items; food/utilities/phone/health derive from the
   chosen lifestyle level; +10% misc buffer; presented as a ±7% range to avoid false precision.
...
- Costs are mid-2026 Calgary point estimates; other cities are directional only.
- Career incomes and ALIS-style market signals are editorial judgment for the prototype, not live data.
...
4. Per-city cost packs (Edmonton, Lethbridge, …).
